...
export default function CreateDIInitiativeModal({ onClose, onCreated }: Props) {
  const [form, setForm] = useState({
    project_name: '',
    tier: TIER_VALUES[2],
    type: TYPE_VALUES[TYPE_VALUES.length - 1],
  ...
  const [trackerInitiativeId, setTrackerInitiativeId] = useState<string | null>(null)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      setError(body.error ?? 'Failed to create. Please try again.')
      setSaving(false)
      return
    }
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            ...
            <p style={{ fontSize: '.68rem', color: 'var(--text-3)', marginTop: '-.5rem', marginBottom: '.75rem' }}>
              Queue # is no longer typed in — it&rsquo;s computed from Priority and RICE Score.
            </p>
            ...
            <textarea placeholder="What does success look like?" value={form.outcome} onChange={e => set('outcome', e.target.value)} rows={2} />
