...
import { statusClass, priorityClass, fmt } from '@/lib/ui'
...
export default function ArchiveTab({ user, canDelete }: Props) {
        ...
        <table>
          ...
          <tbody>
            {filtered.map(i => (
              <tr key={i.id}>
                <td>
                  <div style={{ fontWeight: 700, fontSize: '.78rem', color: 'var(--text)', marginBottom: 2 }}>{i.task_name}</div>
