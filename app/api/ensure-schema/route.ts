...
export async function GET() {
  ...
  // D+I Roadmap (ADR-0001/0002/0003 rebuild — tables are dormant from the prior
  // teardown, CREATE IF NOT EXISTS + ALTER ADD COLUMN IF NOT EXISTS keep this idempotent)
  await sql`
  ...
  `
  await sql`ALTER TABLE di_initiatives ADD COLUMN IF NOT EXISTS queue_position INT`
  await sql`ALTER TABLE di_initiatives ADD COLUMN IF NOT EXISTS size_preset TEXT DEFAULT 'Medium'`
  await sql`
  ...
  await sql`
    INSERT INTO di_config (key, value) VALUES
      ...
      ('wip_cap', '5'),
      ('team_emails', '["cblain@singlethrow.com","dward@singlethrow.com"]'),
